...
if (isset($_POST['update'])) {
    ...
    $stmt = $database->prepare("SELECT img FROM teacher WHERE id = :id");
    ...
    if ($uploadedFileName == null) {
        ...
        if ($update->execute()) {
            ...
            header("location:index.php?success=1&type=success&message=Teacher Updated Successfully", true);
            exit();
        } else {
            echo "not updated <br>";
        }
        ...
        if (getimagesize($_FILES['img']['tmp_name']) !== false) {
            ...
            if (move_uploaded_file($_FILES['img']['tmp_name'], $targetFilePath)) {
                ...
                $update = $database->prepare("UPDATE teacher SET name=:name ,email=:email , phone=:phone ,age=:age , gender=:gender , status=:status, img=:img WHERE id=:id");
                $update->bindParam(":id", $id);
                $update->bindParam(":name", $name);
                $update->bindParam(":email", $email);
                $update->bindParam(":phone", $phone);
