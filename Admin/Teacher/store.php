...
if (isset($_POST['send'])) {
    ...
    if (getimagesize($_FILES['img']['tmp_name']) !== false) {
        if (move_uploaded_file($_FILES['img']['tmp_name'], $targetFilePath)) {
            ...
            $sql = "INSERT INTO teacher (name, email, password, age, status, gender, phone, img , created_by) VALUES (:name, :email, :password, :age, :status, :gender, :phone, :img , :adminID)";
            ...
            if ($stmt->execute()) {
                header("location:index.php?success=1&type=success&message=Teacher Created Successfully", true);
            } else {
