...
if (isset($_POST['update'])) {
    ...
    if ($uploadedFileName == null) {
        $update = $database->prepare("UPDATE specialization SET name=:name , college_id=:collegeID  WHERE id=:id");
        $update->bindParam(":id", $id);
        $update->bindParam(":collegeID", $collegeID);
        ...
        if ($update->execute()) {
            ...
            header("location:index.php?success=1&type=success&message=Specialization Updated Successfully", true);
            exit();
        } else {
            echo "not updated <br>";
        }
        ...
        if (getimagesize($_FILES['img']['tmp_name']) !== false) {
            ...
            if (move_uploaded_file($_FILES['img']['tmp_name'], $targetFilePath)) {
                ...
                $update = $database->prepare("UPDATE specialization SET name=:name , college_id=:collegeID , img=:img WHERE id=:id");
                $update->bindParam(":id", $id);
                $update->bindParam(":name", $name);
                $update->bindParam(":collegeID", $collegeID);
